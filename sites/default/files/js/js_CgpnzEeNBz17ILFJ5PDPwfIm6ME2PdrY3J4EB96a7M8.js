...
(function ($) {
  ...
  $(document).ajaxStart(function (e) {
    let $input = $(e.currentTarget.activeElement);
    let $form = $input.parents('.contact-message-form');
    let $elements = $form.find('.form-control, .button');
...
(function ($) {
  ...
  Vue.component('product-gallery', {
    ...
    methods: {
      // Получаем объект с галереей.
      getGallery() {
        axios.post('/api/product/get-gallery/' + this.id).then(response => {
          this.loading = false;
...
(function ($) {
  ...
  function blankLinks() {
    $('.text-formatted a, .field--name-field-attach a, #contacts a').each(function () {
